...
let calibrationArray = calibrationList.split("\n  "); //coloca cada string em uma posição de array
let calibrationStringfiedNumberArray = calibrationArray
  .map((string) => string.match(/\d+/g))
  .filter(Boolean); //cria uma array com várias arrays de números, mas esses números ainda estão no formato de string. Preciso agora extrair esses números de cada uma das strings e, depois, tranformá-los em Number.

let onlyNumbersInStrings = calibrationStringfiedNumberArray.map((array) =>
  array.toString()
);

let onlyFirst = onlyNumbersInStrings.map((index) => index.slice(0, 1)); //pega apenas o primeiro caracter da string (no caso, justamente o primeiro numero, que é o que quero), e me retorna um array com apenas esses numeros.
let onlyLast = onlyNumbersInStrings.map((index) => index.slice(-1)); //pega apenas o último caracter da string (no caso, justamente o último numero, que é o que quero), e me retorna um array com apenas esses numeros.

let concatenando = [];

for (let i = 0; i < 1000; i++) {
  let nova = parseInt(onlyFirst[i] + onlyLast[i]);
  concatenando.push(nova);
}

const sum = concatenando.reduce((accumulator, currentValue) => {
